...
export default class FileImage {
  ...
  public async Save_Thumb_image(url: Image): Promise<string | null> {
    ...
    const source:string = path.resolve(this.images, `${url.filename}.jpg`)
    const width: string = url.width as string
    const heigth: string = url.height as string
    const process = new ImageProcessing()
    const result:string|null = await process.image_resize(
      source,
      distination,
      width,
      heigth
  ...
  public async cashed_image(url: Image): Promise<string | null> {
    const cashed: string = path.resolve(
      this.image_thumb,
      `${url.filename}${url.width}${url.height}-Thumb.jpg`
    )

    const check = new Check()
    const Path_exists:string|null = await check.checkFile(
      path.resolve(
        this.image_thumb,
        `${url.filename}${url.width}${url.height}-Thumb.jpg`
      )
